...
async function main() {
  ...
  try {
    console.log(`====================================================================`);
    console.log(`GARAGEKINGS BACKFILL: STANDARD CATALOG PRODUCTS ONLY (46 ITEMS)`);
    console.log(`====================================================================\n`);
    ...
    // Filter ONLY standard products (isPrebook === false)
    const standardProducts = catalog.products.filter(p => !p.isPrebook);
    console.log(`Found ${standardProducts.length} standard catalog products to backfill.\n`);
    ...
    for (let i = 0; i < standardProducts.length; i++) {
      const p = standardProducts[i];
      const sku = p.sku;

      if (existingSkus.has(sku)) {
        console.log(`[${i + 1}/${standardProducts.length}] ⏭️ Skipping SKU "${sku}" (Already exists).`);
        skippedCount++;
        continue;
      }

      console.log(`[${i + 1}/${standardProducts.length}] Backfilling standard SKU "${sku}" (${p.brand} - ${p.name})...`);
      ...
      `, [
        ...
        variantObj.salesStatus || 'Available',
      ...
      console.log(`  ✅ Successfully backfilled standard product SKU "${sku}".`);
    }

    console.log(`\n🎉 STANDARD PRODUCTS BACKFILL COMPLETE!`);
    console.log(`- Inserted Standard Products: ${insertedCount}`);
